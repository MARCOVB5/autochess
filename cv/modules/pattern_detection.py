...
def load_empty_board_reference(reference_path="cv/assets/chessboard_nopieces.png"):
    """
    Carrega a imagem de referência do tabuleiro vazio e a prepara para uso na detecção de peças.
    
    Args:
        reference_path: Caminho para a imagem de referência
        
    Returns:
        Imagem de referência processada ou None se falhar
    """
    global empty_board_reference, empty_board_squares
    
    # Carregar a imagem de referência
    reference_img = cv2.imread(reference_path)
    
    if reference_img is None:
        print(f"❌ Não foi possível carregar a imagem de referência: {reference_path}")
        return None
    
    # Detectar o tabuleiro e aplicar a mesma transformação que aplicaremos em imagens atuais
    corners, _ = detect_board_corners(reference_img)
    
    if corners is None:
        print("❌ Falha ao detectar os cantos no tabuleiro de referência.")
        return None
    
    # Aplicar transformação de perspectiva
    warped_reference, _ = warp_board_perspective(reference_img, corners)
    
    # Converter para escala de cinza
    gray_reference = cv2.cvtColor(warped_reference, cv2.COLOR_BGR2GRAY)
    
    # Armazenar a referência globalmente
    empty_board_reference = warped_reference
    
    # Dividir a referência em células individuais
    empty_board_squares = split_board_into_squares(warped_reference)
...
def detect_piece_in_square(square_img, square_position=None):
    """
    Detecta se há uma peça no quadrado usando subtração de fundo e identifica sua cor.
    
    Args:
        square_img: Imagem do quadrado individual
        square_position: Posição (linha, coluna) do quadrado no tabuleiro
        
    Returns:
        contains_piece: Booleano indicando presença de peça
        piece_color: 'white', 'black' ou None
        piece_info: Dicionário com informações adicionais sobre a peça
    """
    global empty_board_reference, empty_board_squares
    ...
    # Converter para escala de cinza
    cell_gray = cv2.cvtColor(square_img, cv2.COLOR_BGR2GRAY)
    
    # Se temos a referência do tabuleiro vazio E a posição do quadrado
    if empty_board_squares is not None and square_position is not None:
        # Encontrar o quadrado correspondente na referência
        ref_square = None
        for sq in empty_board_squares:
            if sq['position'] == square_position:
                ref_square = sq
                break
        
        if ref_square is not None:
            # Converter a imagem de referência para escala de cinza
            bg_gray_cell = cv2.cvtColor(ref_square['image'], cv2.COLOR_BGR2GRAY)
            
            # Garantir que as imagens tenham o mesmo tamanho
            if cell_gray.shape != bg_gray_cell.shape:
                bg_gray_cell = cv2.resize(bg_gray_cell, (cell_gray.shape[1], cell_gray.shape[0]))
            
            # 1. Detectar presença de peça
            # Calcular a diferença absoluta entre a célula atual e a referência
            diff = cv2.absdiff(cell_gray, bg_gray_cell)
            
            # Aplicar threshold binário para destacar diferenças significativas
            TH_PRESENCE = 25  # Limiar para diferença significativa
            _, mask = cv2.threshold(diff, TH_PRESENCE, 255, cv2.THRESH_BINARY)
            
            # Aplicar operação morfológica para remover ruídos
            kernel = np.ones((3, 3), np.uint8)
            mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)
            
            # Contar pixels brancos na máscara
            white_pixels = cv2.countNonZero(mask)
            
            # Definir um limiar para decidir se há uma peça
            MIN_PIXELS = int(cell_gray.shape[0] * cell_gray.shape[1] * 0.05)  # 5% da área
            has_piece = white_pixels > MIN_PIXELS
            
            piece_info = {
                'white_pixels': white_pixels,
                'min_pixels': MIN_PIXELS,
                'threshold': TH_PRESENCE,
                'diff_mask': mask.copy()
            }
            
            # 2. Se não tiver peça, retornar
            if not has_piece:
                return has_piece, None, piece_info
            
            # 3. Classificar cor usando HSV
            cell_hsv = cv2.cvtColor(square_img, cv2.COLOR_BGR2HSV)
            
            # Calcular margens para crop (10% de cada lado)
            h, w = cell_hsv.shape[:2]
            CROP = int(h * 0.1)  # 10% da borda
            CROP2 = h - CROP  # 90% do tamanho (10% da outra borda)
            
            # Garantir que o crop é válido
            if CROP2 <= CROP or CROP < 0 or CROP2 > h:
                CROP = max(0, min(int(h * 0.1), h // 4))
                CROP2 = min(h, max(int(h * 0.9), h // 2))
            
            # Recortar região central
            inner = cell_hsv[CROP:CROP2, CROP:CROP2]
            
            # Calcular média do canal V e mediana do canal S
            mean_v = inner[:,:,2].mean()
            med_s = np.median(inner[:,:,1])
            
            # Definir limiares para classificação
            V_TH = 125  # Limiar para valor/brilho
            S_TH = 60   # Limiar para saturação
            
            # Classificar com base nos limiares
            if mean_v > V_TH and med_s < S_TH:
                piece_color = 'white'  # Peça branca
            else:
                piece_color = 'black'  # Peça preta
            ...
            piece_info.update({
                ...
                'v_threshold': V_TH,
                's_threshold': S_TH,
                'classification_rule': f"V_mean > {V_TH} and S_median < {S_TH}"
            })
            
            return has_piece, piece_color, piece_info
    
    # Se não temos referência ou a referência não foi encontrada, usar método fallback
    # Aplicar detecção básica por contornos
    blurred = cv2.GaussianBlur(cell_gray, (5, 5), 0)
    ...
    min_area = square_img.shape[0] * square_img.shape[1] * 0.1
    large_contours = [c for c in contours if cv2.contourArea(c) > min_area]
    
    # Se não há contornos grandes, não há peça
    if not large_contours:
        return False, None, piece_info
    
    # Há peça - tentar classificar por HSV
    cell_hsv = cv2.cvtColor(square_img, cv2.COLOR_BGR2HSV)
    ...
    return True, piece_color, piece_info
...
def process_board_image(img):
    ...
    # Carregar a imagem de referência do tabuleiro vazio se ainda não foi carregada
    if empty_board_reference is None:
        load_empty_board_reference()
