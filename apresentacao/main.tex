...
\begin{itemize}
  \item Motores não se movem junto.
...
\begin{frame}[plain]{\textit{Core}}

\begin{itemize}
    \item Um programa denominado \textit{Core} ficou encarregado da integração entre visão computacional, IA e o movimento da CNC.
     \item É o cérebro do projeto.
\end{itemize}
...
\begin{frame}{Resultados - decisões que funcionaram bem}
  ...
  \item \textbf{Hardware}:
    ...
    \item \textbf{Mecânica}:
        \begin{itemize}
            \item Estrutura do braço CNC forte e confiável (desvio médio inferior a 2mm);
...
\begin{frame}{Resultados - problemas a serem melhorados}
  ...
  \item \textbf{Hardware}:
    ...
    \item \textbf{Software}:
        \begin{itemize}
            \item Visão Computacional tomando muito tempo para processamento de tabuleiro cheio (média de 3,2 segundos).
        \end{itemize}
  \end{itemize}
\end{frame}
